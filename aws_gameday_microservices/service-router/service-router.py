...
def update_routes():
    ...
    try:
        ...
        for _ in range(5):
            services = table.scan()
        # need to declare its a global to modify it, cause threading
        global ROUTES
        ROUTES = {}

        for service in services["Items"]:
            if service["ServiceType"] not in ROUTES.keys():
                ROUTES[service["ServiceType"]] = [
                    service["Endpoint"],
                ]
...
def api():
    ...
    if (
        request.method == "POST"
    ):  # This code deals with API calls, and sends to microservice chain
        ...
        json_log(
            "Host {} received RequestId {}, asking for services: {}".format(
                socket.gethostname(),
                messageRequest["RequestId"],
                ",".join(messageRequest["Services"]),
            )
        )
        ...
        for service in messageRequest["Services"]:
            ...
            try:
                ...
                json_log(
                    "RequestId {} needs Service:{} using Endpoint:{}".format(
                        messageRequest["RequestId"], service, service_endpoint
                    )
                )
                req = requests.post(
                    service_endpoint,
                ...
                json_log(
                    "Request included a service we dont support, Shutting down server",
                    "error",
                )
                json_log(
                    "service-router may need HA config, or to run the latest code from the dashboard",
                    "warning",
                )
                ...
                json_log(
                    'Error:"{}" connecting to Service:{} Endpoint:{}'.format(
                        e, service, service_endpoint
                    ),
                    "exception",
                )
                errors += 1
            try:
                routerResponse["Responses"].append(
                    req
                )  # Add the microservice response to our list of responses
                messageRequest["Message"] = req["Message"]
            except Exception as e:
                json_log(
                    'Bad response:"{}" from Service:{} Endpoint:{}'.format(
                        e, service, service_endpoint
                    ),
                    "exception",
                )
        ...
        if errors:
            json_log(
                "RequestId {} completed with {} errors".format(
                    messageRequest["RequestId"], str(errors)
                )
            )
        else:
            json_log(
                "RequestId {} completed successfully".format(
                    messageRequest["RequestId"]
                )
            )
...
if __name__ == "__main__":
    ...
    try:
        xray_recorder.configure(
            service="service-router", sampling=False, context_missing="LOG_ERROR"
        )
    ...
    json_log(
        "New service-router instance {} has come online".format(socket.gethostname())
    )
